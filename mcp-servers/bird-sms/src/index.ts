...
server.setRequestHandler(CallToolRequestSchema, async (request) => {
  ...
  try {
    switch (name) {
      case 'bird_send_sms': {
        const { to, message, channelId } = args as {
          to: string;
          message: string;
          channelId?: string;
        };
        ...
        const result = await birdRequest(
      ...
      case 'bird_send_via_navigator': {
        const { navigatorId, to, message } = args as {
          navigatorId: string;
          to: string;
          message: string;
        };
        ...
        const result = await birdRequest(
          `/workspaces/${BIRD_WORKSPACE_ID}/navigators/${navigatorId}/messages`,
          'POST',
          {
            receiver: {
              contacts: [{ identifierValue: formatPhoneNumber(to) }],
      ...
      case 'bird_get_message': {
        const { messageId, channelId } = args as {
          messageId: string;
          channelId?: string;
        };
      ...
      case 'bird_list_navigators': {
        const result = await birdRequest(
          `/workspaces/${BIRD_WORKSPACE_ID}/navigators`
        );
      ...
      case 'bird_send_template': {
        const { to, templateId, parameters, channelId } = args as {
          to: string;
          templateId: string;
