...
-- FIXED: Updated RLS policies to match actual schema (users table, not organization_members)
...
-- These are stored encrypted in the settings JSONB field
...
CREATE TABLE IF NOT EXISTS organization_minds (
  ...
  -- Local references
  user_id UUID REFERENCES auth.users(id) ON DELETE SET NULL, -- Optional: linked to a specific user
  team_id UUID REFERENCES teams(id) ON DELETE SET NULL, -- Optional: linked to a specific team
...
-- GIN index for JSONB psychometric profile queries
...
CREATE TABLE IF NOT EXISTS mind_simulations (
  ...
  -- Context
  context_type TEXT, -- 'assessment', 'outreach', 'coaching', etc.
  context_id UUID, -- Reference to assessment, outreach campaign, etc.
...
SELECT
  ...
  jsonb_object_agg(
    COALESCE(om.psychometric_profile->>'communicationStyle', 'unknown'),
    COUNT(*)
  ) FILTER (WHERE om.psychometric_profile->>'communicationStyle' IS NOT NULL) AS communication_styles,

  -- AI readiness distribution
  jsonb_object_agg(
    COALESCE(om.psychometric_profile->>'aiReadiness', 'unknown'),
    COUNT(*)
  ) FILTER (WHERE om.psychometric_profile->>'aiReadiness' IS NOT NULL) AS ai_readiness_dist,

  -- Change readiness distribution
  jsonb_object_agg(
    COALESCE(om.psychometric_profile->>'changeReadiness', 'unknown'),
    COUNT(*)
  ) FILTER (WHERE om.psychometric_profile->>'changeReadiness' IS NOT NULL) AS change_readiness_dist,
...
-- FIXED: Using users table with organization_id column instead of organization_members
...
-- FIXED: Check role from users table
...
-- Function to get org's Mind Reasoner API key (decrypted)
...
DECLARE
  api_key TEXT;
BEGIN
  -- Check if user has access to this org (using users table)
  IF NOT EXISTS (
    SELECT 1 FROM users
  ...
  -- TODO: In production, decrypt the key here using pgsodium or Vault
  RETURN api_key;
END;
$$;

-- Function to get mind count by type for an organization
...
-- DROP FUNCTION IF EXISTS get_org_mind_stats(UUID);
-- DROP FUNCTION IF EXISTS get_org_mind_reasoner_key(UUID);
-- DROP POLICY IF EXISTS "Org admins and team leads can create simulations" ON mind_simulations;
-- DROP POLICY IF EXISTS "Users can view simulations in their organization" ON mind_simulations;
-- DROP POLICY IF EXISTS "Org admins can manage minds" ON organization_minds;
-- DROP POLICY IF EXISTS "Users can view minds in their organization" ON organization_minds;
-- DROP VIEW IF EXISTS team_psychometric_summary;
-- DROP INDEX IF EXISTS idx_mind_simulations_created;
-- DROP INDEX IF EXISTS idx_mind_simulations_context;
-- DROP INDEX IF EXISTS idx_mind_simulations_mind;
-- DROP INDEX IF EXISTS idx_mind_simulations_organization;
-- DROP TABLE IF EXISTS mind_simulations;
-- DROP INDEX IF EXISTS idx_org_minds_psychometric_profile;
-- DROP INDEX IF EXISTS idx_org_minds_active;
-- DROP INDEX IF EXISTS idx_org_minds_type;
-- DROP INDEX IF EXISTS idx_org_minds_team;
-- DROP INDEX IF EXISTS idx_org_minds_user;
-- DROP INDEX IF EXISTS idx_org_minds_organization;
-- DROP TRIGGER IF EXISTS update_organization_minds_updated_at ON organization_minds;
-- DROP TABLE IF EXISTS organization_minds;
-- DROP INDEX IF EXISTS idx_organizations_mind_reasoner_enabled;
-- ALTER TABLE organizations DROP COLUMN IF EXISTS mind_reasoner_configured_at;
-- ALTER TABLE organizations DROP COLUMN IF EXISTS mind_reasoner_enabled;
-- ALTER TABLE organizations DROP COLUMN IF EXISTS mind_reasoner_workspace_id;
-- ALTER TABLE organizations DROP COLUMN IF EXISTS mind_reasoner_api_key_encrypted;
