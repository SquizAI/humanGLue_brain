...
CREATE POLICY "Org admins can create simulations"
  ON mind_simulations FOR INSERT
  WITH CHECK (
    organization_id IN (
      SELECT organization_id FROM organization_members
      WHERE user_id = auth.uid() AND role IN ('admin', 'org_admin', 'team_lead')
    )
  );
...
-- Note: In production, use proper encryption/vault service
